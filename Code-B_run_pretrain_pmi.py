...
norm_pmi_rank_dict= pickle.load(open('/pmi/pmi_data/seq_to_pmi_rank.pkl', "rb"))
...
def kmer_ids_to_sequence(ids, dna_tokenizer):
    return dna_tokenizer.decode(ids, skip_special_tokens=True)

def kmer_id_to_pmi_rank(id, dna_tokenizer):
    kmer_sequence1= dna_tokenizer.decode(id, skip_special_tokens=True).split()
    kmer_sequence= ''.join(kmer_sequence1)

    #print('Inside kmer id function: ', id, kmer_sequence )
    try:
        pmi_rank= norm_pmi_rank_dict[kmer_sequence]
    except KeyError:
        pmi_rank= 4000
    return pmi_rank
    

def kmer_position_to_real_index(kmer_position, k):
    return kmer_position

def find_pmi_token_for_kmer(ids, position, pmi_tokenizer, dna_tokenizer):
    # get actual sequence
    kmer_sequence = kmer_ids_to_sequence(ids, dna_tokenizer)
    # print('Kmers sequences: ', kmer_sequence)
    
    kmers = kmer_sequence.split()
    # print('Kmers: ', kmers)

    org_sequence = pmi_tokenizer._kmer2seq(kmer_sequence)
...
def mask_tokens(inputs: torch.Tensor, tokenizer: PreTrainedTokenizer, args) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    mask_list_11= [-5, -4, -3, -2, -1, 1, 2, 3, 4, 5]
    ...
    for i, masked_index in enumerate(masks):
        ...
        #print('Total number of mask centers= ', len(mask_centers))
        new_centers = deepcopy(mask_centers)

        highest_pmi_id_per_mask_center= []

        for center in mask_centers:
            min_pmi_rank= kmer_id_to_pmi_rank(center, dna_tokenizer=tokenizer)
            min_pmi_index= center
            ...
            for mask_number in mask_list:
                ...
                if current_index <= end and current_index >= 1:
                    ...
                    current_pmi_rank= kmer_id_to_pmi_rank(current_index, dna_tokenizer=tokenizer)
                    if current_pmi_rank < min_pmi_rank:
                        min_pmi_rank= current_pmi_rank
                        min_pmi_index= current_index
            highest_pmi_id_per_mask_center.append((min_pmi_rank, min_pmi_index))

        
        if highest_pmi_id_per_mask_center:
            segment_cnt= math.ceil(len(highest_pmi_id_per_mask_center)/2)
        ...
        #print('Masked token percentage= ', len(new_centers)*100/512)
        
        # new_centers is the list containing all the masked indices, 15% of 512 tokens
